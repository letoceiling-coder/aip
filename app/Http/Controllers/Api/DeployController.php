    ...
    protected function handleComposerInstall(): array
    {
        try {
            ...
            if (empty($composerPath) || $composerPath === 'composer') {
                // Если путь не найден или это просто 'composer', пробуем использовать через which
                // или используем composer как команду напрямую
                $command = "composer install --no-dev --optimize-autoloader --no-interaction --no-scripts";
            } else {
                // Если найден полный путь, используем PHP для выполнения composer файла
                // Это обходит проблему с правами на выполнение, так как PHP читает и выполняет файл
                $escapedPath = escapeshellarg($composerPath);
                
                // Используем PHP для выполнения composer (наиболее надежный способ)
